create table if not exists `t_end_projects`(
    `id` bigint auto_increment,
    `name` varchar(100) not null comment '项目名称',
    `desc` varchar(255) comment '描述',
    `owner` varchar(20) not null comment '项目所有者',
    `created_time` datetime  comment '创建时间',
    `modified_time` datetime comment '修改时间',
    primary key(`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;

create table if not exists `t_end_cases`(
    `id` bigint auto_increment,
    `project_id` bigint not null comment '关联的项目',
    ...
    `created_time` datetime  comment '创建时间',
    `modified_time` datetime comment '修改时间',
    `is_before` int default 0  comment "是否是前置处理脚本 0 不是 1 是",
    `is_after` int default 0  comment "是否是后置处理脚本 0 不是 1 是",
    primary key(`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;

create table if not exists `t_before_exec_cases`(
    `id` bigint auto_increment,
    `project_id` bigint not null comment '关联的项目名',
    `before_case_id` bigint comment '前置脚本',
    `exec_case_id` bigint comment '正式执行的脚本',
    primary key(`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;

create table if not exists `t_after_exec_cases`(
    `id` bigint auto_increment,
    `project_id` bigint not null comment '关联的项目',
...
create table if not exists `t_end_confs`(
     `id` bigint auto_increment,
     `project_id` bigint not null comment '关联的项目',
